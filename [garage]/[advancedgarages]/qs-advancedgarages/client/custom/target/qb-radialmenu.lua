...
local function AddRadialOption()
    ...
    if ClosestGarage and not inVehicle and garage.available or IsGarageOwner or IsKeyHolder then
        ...
        if garage.type ~= 'boat' and not garage.isImpound and not inVehicle then
            table.insert(menuItems, {
                id = 'radialEnterShell',
                title = 'Enter Garage',
                icon = 'warehouse',
...
function RemoveRadialOptions()
    ...
    DebugPrint('The qb-radialmenu table has been cleaned!')
end
