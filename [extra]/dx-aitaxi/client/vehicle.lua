...
function CreateTaxiPed(vehicle)
	...
	if DoesEntityExist(vehicle) then
		if IsModelValid(model) then
			...
			local ped = CreatePedInsideVehicle(vehicle, 26, model, -1, false, false)
	...
	if IsModelValid(taxiModel) then
		if IsThisModelACar(taxiModel) then
			...
			if not DoesEntityExist(taxiVeh) then
				...
				DisplayNotify("Call Taxi", "Taxi Driver is on his way")
				...
				DisplayNotify("Call Taxi", "All taxi drivers are busy.")
			end
		end
	end	
end
