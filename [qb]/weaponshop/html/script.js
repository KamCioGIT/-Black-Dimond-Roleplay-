...
$(document).on("click", ".swiper-slide", function (e) { 
    ...
    $.each(WeaponTable[SelectedType], function (i, result) {
        if (result.hash == weapon) {
            $.post("http://weaponshop/ChangeWeaponObjects", JSON.stringify({weapon : weapon}));
            ...
            if (curWeapon !== "") {
                if (curWeapon == weapon) {
                    ...
                    $.post("http://weaponshop/OnWeaponChange", JSON.stringify({}));
                }
            }
            curWeapon = weapon
...
$(document).on("click", ".weapon-attachment-selected", function (e) { 
    ...
    $.post("http://weaponshop/AddBasket", JSON.stringify({weapon: curWeapon, attachment : SelectedAttachmentTypei, SelectedAttachment : SelectedAttachment,attachmentData : WeaponAttachmentsData[curWeapon][SelectedAttachmentTypei][SelectedAttachment]}));
    $.post("http://weaponshop/ChangeWeaponAttachment", JSON.stringify({weapon : curWeapon, attachment : WeaponAttachmentsData[curWeapon][SelectedAttachmentTypei][SelectedAttachment]["component"]}));
    SelectedWeaponAttachment = ""
    $(".weapon-attachment-selected").html('<i class="fas fa-plus"></i>')
    $("#attachment-name").html("-")
    $("#attachment-index").html("-")
...
$(document).on("click", ".weapon-skins-and-tints-index", function (e) {
    ...
    $(".weapon-tints-gobackbutton").fadeOut(300)
    $.post("http://weaponshop/AddTintorSkin", JSON.stringify({type : $(this).attr("type"), mk2 : mk2, id : $(this).attr("id"), weapon : curWeapon  }));
})
...
$(document).on("mouseenter", ".weapon-skins-and-tints-index", function(e){
    ...
    if (indexType == "skin") {
        $.post("http://weaponshop/ChangeTintorSkin", JSON.stringify({type : $(this).attr("type"), skin : $(this).attr("id"), weapon : curWeapon, index : "check"}));
    } else {
        $.post("http://weaponshop/ChangeTintorSkin", JSON.stringify({type : $(this).attr("type"), tint : $(this).attr("id"), weapon : curWeapon, index : "check"}));
    }
...
//             $.post("http://weaponshop/AddTintorSkin", JSON.stringify({type : "skin", mk2 : mk2, id : $(this).attr("value"), weapon : curWeapon  }));
//             // $.each(WeaponTable[SelectedType], function (i, result) {
//             //     if (result.hash == curWeapon) {
...
//             //     }
//             // })
//             $.post("http://weaponshop/ChangeTintorSkin", JSON.stringify({type : "skin", skin : $(this).attr("value"), weapon : curWeapon}));
//             // SelectedSkin = value
//             // if (SelectedTint != "") {
//             //     if (mk2) {
...
//             //$("#test").html('<div class="swiper-slide" id="secswiper" value="skin" ><div class="weapon-tints-tint-index">Weapon Skins</div><div class="bottom-line"></div></div><div class="swiper-slide" id="tt" value="tints" style="background: url(images/weapon_tints.png); background-repeat: no-repeat; background-size: cover;"><div class="weapon-tints-tint-index">Tints</div><div class="bottom-line"></div></div>')
//             $(".weapon-tints-gobackbutton").fadeOut(300)
//             $.post("http://weaponshop/AddTintorSkin", JSON.stringify({type : "tint", mk2 : mk2, id : $(this).attr("value"), weapon : curWeapon  }));
//             // if (mk2) {
...
//             // }
//             $.post("http://weaponshop/ChangeTintorSkin", JSON.stringify({type : "tint", tint : $(this).attr("value"), weapon : curWeapon}));
...
HSN.SendNuiMessage = function(message) {
    $.post("http://weaponshop/SendMessage", JSON.stringify({message : message}));
}
...
$(document).on("click", ".weapon-attachment-remove-image", function (e) { 
    $.post("http://weaponshop/RemoveBasket", JSON.stringify({attachment : $(this).attr("attachmenttype"), componenthash : $(this).attr("componenthash"), weapon: curWeapon,  SelectedAttachment : $(this).attr("attachment")}));
})
...
$(document).on("click", ".weapon-buybutton", function (e) { 
    HSN.Close();
    $.post("http://weaponshop/BuyWeapon", JSON.stringify({weapon : curWeapon}));
})
