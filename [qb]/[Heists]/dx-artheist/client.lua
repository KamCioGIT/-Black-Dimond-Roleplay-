    ...
    while true do
        ...
        if heistDist <= 3.0 then
            ...
            if IsControlJustPressed(0, 38) then
                QBCore.Functions.TriggerCallback('artheist:server:getCops', function(cops)
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            if weapon == GetHashKey('WEAPON_SWITCHBLADE') then
                                if not ArtHeist['cuting'] then
                                    ...
                                    QBCore.Functions.Notify(Strings['already_cuting'])
                                end
                            else
...
function StartHeist()
    QBCore.Functions.TriggerCallback('artheist:server:checkRobTime', function(time)
        if time then
            if ArtHeist['start'] then QBCore.Functions.Notify(Strings['already_heist']) return end
            ...
            QBCore.Functions.Notify(Strings['go_steal'])
