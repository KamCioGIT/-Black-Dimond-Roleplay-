...
local hackingPosition = vector3(1272.7, -1711.34, 54.77)
...
RegisterNetEvent('dx-gunheist:manipulateDrop')
AddEventHandler('dx-gunheist:manipulateDrop', function(dropId, dropData)
...
RegisterNetEvent('dx-gunheist:obtainDrops')
AddEventHandler('dx-gunheist:obtainDrops', function(serverDrops)
	activeDrops = serverDrops
end)

RegisterNetEvent('dx-gunheist:createBlipOnDrop')
AddEventHandler('dx-gunheist:createBlipOnDrop', function(dropId)
...
RegisterNetEvent('dx-gunheist:openCrate')
AddEventHandler('dx-gunheist:openCrate', function(crateObject)
...
Citizen.CreateThread(function()
	TriggerServerEvent('dx-gunheist:obtainDrops')
...
AddEventHandler('do', function(dropId)
	...
	TriggerEvent('dx-gunheist:openCrate', activeDrops[dropId].object)
	...
	QBCore.Functions.Progressbar("drilling_", "Drilling...", math.random(20000, 30000), false, true, {}, {}, {}, {}, function()
		TriggerServerEvent('dx-gunheist:pickUpDrop', dropId)
		inAnimation = false
	end)
end) 
...
RegisterNetEvent('dx-gunheist:useChip')

AddEventHandler('dx-gunheist:useChip', function()
	local dstcheck = #(GetEntityCoords(PlayerPedId()) - hackingPosition)

	if dstcheck < 5 then
		if StartHacking() then
			TriggerServerEvent('dx-gunheist:wonHacking')
		end
	end
end)
...
AddEventHandler('dx-gunheist:doDecryptionAnimation', function()
	local ped = PlayerPedId()
	RequestAnimDict('missheist_jewel@hacking')
	TaskPlayAnim(ped, "missheist_jewel@hacking", "hack_loop", 8.0, 1.0, -1, 9, 0, 0, 0, 0)
end)

RegisterNetEvent('dx-gunheist:useDecryptionKey')
AddEventHandler('dx-gunheist:useDecryptionKey', function(item)
	local dstcheck = #(GetEntityCoords(PlayerPedId()) - hackingPosition)
	-- print(item)
	if dstcheck < 5 then
		TriggerEvent('dx-gunheist:doDecryptionAnimation')
		...
		QBCore.Functions.Progressbar("drilling_", "Decrypting with key...", math.random(10000, 15000), false, true, {}, {}, {}, {}, function()
			...
			TriggerServerEvent('dx-gunheist:server:useDecryptionKey', item)
		end)
	end
end)
...
function hackingCompleted(success, timeremaining)
	if success then	
		TriggerServerEvent('dx-gunheist:hackingCompleted')
	else
		local coords = vector3(1275.639, -1710.568, 54.771)
		--PoliceAlert(coords)
		exports['ps-dispatch']:WeaponHeist()
		QBCore.Functions.Notify("Your hacking is failed. This will alert Police. Hurry up!", "error") 
	end
...
RegisterNetEvent('dx-gunheist:hackingMinigame')
AddEventHandler('dx-gunheist:hackingMinigame', function()
	TriggerEvent("mhacking:show")
	TriggerEvent("mhacking:start", 7, 15, hackingCompleted)
end)
