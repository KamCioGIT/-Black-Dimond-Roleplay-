    ...
    if xPlayer.PlayerData.money.cash >= Config.Pickaxe[pickaxe+1].price then
        ...
        xPlayer.Functions.RemoveMoney('cash', Config.Pickaxe[pickaxe+1].price)
            ...
            if result[1] ~= nil then
                print('work7')
                exports.oxmysql:execute('UPDATE dream_jobs SET pickaxe = @pickaxe WHERE identifier=@identifier AND job=@job', {
                    ['@pickaxe']   = pickaxe+1,
                    ['@identifier'] = xPlayer.PlayerData.citizenid,
                    ['@job'] = 'miner',
                })
...
QBCore.Functions.CreateCallback('dream-miner:payout', function(source, cb, exp)
    ...
    if ore > 5 then
        ...
        if result[1] ~= nil then
            exports.oxmysql:execute('UPDATE dream_jobs SET experience = @experience WHERE identifier=@identifier AND job=@job', {
...
QBCore.Functions.CreateCallback('dream-miner:getdatabase', function(source, cb, xPlayer)
    ...
    if result[1] ~= nil then
        ...
        exports.oxmysql:execute('INSERT INTO dream_jobs (identifier,experience,pickaxe,job) VALUES (@identifier,@experience,@pickaxe,@job)', {
            ['@identifier'] = xPlayer.PlayerData.citizenid, 
            ['@experience'] = 0,
            ['@pickaxe'] = 1,
            ['@job'] = 'miner',
