...
function ColdDown()
    ...
    QBCore.Functions.Notify("Cool Down:"..tostring(isColddown) , "error")
    -- 600000 (10 minutos)
    Citizen.Wait(300000)
    isColddown = false
    QBCore.Functions.Notify("Cool Down:"..tostring(isColddown) , "error")
...
function getNewLocation()
    ...
    if location ~= 0 then
        QBCore.Functions.Notify("Go to next location - "..location)
...
function TouchProcess()
    if not BuilderData.ShowDetails then
        ...
        QBCore.Functions.Progressbar("touch_process", "Repairing ..", math.random(6000,8000), false, true, {
            ...
            QBCore.Functions.Notify("Done!", "success", 4000)
            ClearPedTasks(PlayerPedId())
        end, function() -- Cancel
            ClearPedTasks(PlayerPedId())
            TriggerEvent('qb-telco:client:SetTaskState', BuilderData.CurrentTask, false, false)
        ...
        if PlayerJob.name == "telco" then
            ...
            if MainDistance < 10 then
                ...
                if IsControlJustPressed(0, 38) then
                    QBCore.Functions.Notify('Colddown value: '..tostring(isColddown) , 'error')
