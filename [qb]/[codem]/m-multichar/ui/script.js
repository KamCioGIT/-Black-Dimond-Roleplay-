...
const app = Vue.createApp({
    ...
    methods: {
        ...
        shouldShowCharacter(slotId) {
            ...
            return !this.viewedCharacters.includes(slotId) && this.getCharacter(slotId);
        ...
        getCharacter(slotId, bool) {
            ...
            if (!this.loadingContainer) {
                if (bool) {
                    const character = this.charactersData.find(info => info.cid === slotId);
                    if (character) {
                        return character;
                    }
                    return null;
        ...
        createCharacter() {
            ...
            for (const value of [
              ...
              const dateValue = new Date(this.selectedDate); // input değerini Date nesnesine dönüştür
             const year = dateValue.getFullYear(); // yıl bilgisini al
        ...
        deleteCharacter() {
            const charactersDatas = this.charactersData.find(item => item.citizenid == this.currentCharacterData.citizenid);
            postNUI("DeleteCharacter",{
                 citizenid: this.currentCharacterData.citizenid
                }
            )
            this.charactersData.splice(this.charactersData.indexOf(charactersDatas), 1);
            this.currentCharacterData = false
            this.deletePopup = false
        },
        checkData() {
    ...
    computed: {
        formattedAge() {
            const birthdateArr = this.currentCharacterData.charinfo.birthdate.split("-"); 
            ...
            const formattedBirthdate = birthdateObj.toLocaleDateString(this.dateFormat, {
              day: "numeric",
              month: "long",
              year: "numeric",
            }); 
