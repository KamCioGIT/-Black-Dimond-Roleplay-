...
local dealerpos = { x = 448.21, y = -898.61, z = 27.68, h = 250.4 }
	...
	while true do 
		...
		if #(pos - vector3(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z)) < 5.0 then
			...
			if #(pos - vector3(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z)) < 1.5 then
				if GetClockHours() >= 18 and GetClockHours() <= 23 then
					if not sellItemsSet then 
						sellPrice = GetSellingPrice()
						sellItemsSet = true
					elseif sellItemsSet and sellPrice ~= 0 then
						DrawText3D(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z, "Do you have someting to sell? Press ~g~E~w~")
						if IsControlJustReleased(0, 38) then
							TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_STAND_IMPATIENT", 0, true)
                            QBCore.Functions.Progressbar("sell_coke_items", "Selling Items", math.random(45000,60000), false, true, {}, {}, {}, {}, function() -- Done
						...
						DrawText3D(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z, " You have nothing to sell, Fuck off!!!")
					end
				else
					DrawText3D(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z, " I don't know what are you talking, come in the ~r~evening~w~, Now fuck off!!")
				end
			end
		end
		if not inRange then
