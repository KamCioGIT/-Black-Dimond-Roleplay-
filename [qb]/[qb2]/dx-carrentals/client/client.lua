...
Citizen.CreateThread(function()
    ...
    for i = 1, #CRConfig.RentingPositions, 1 do
		...
		SetVehicleNumberPlateText(veh, i .. "GRNAW")
        ...
        local pos = GetEntityCoords(PlayerPedId(), true)
			local ShopDistance = GetDistanceBetweenCoords(pos, CRConfig.RentalSpots[ClosestShop].coords.x, CRConfig.RentalSpots[ClosestShop].coords.y, CRConfig.RentalSpots[ClosestShop].coords.z, false)
				if ShopDistance < 50 then
					...
					--Draw3DText(CRConfig.RentalSpots[ClosestShop].coords.x, CRConfig.RentalSpots[ClosestShop].coords.y, CRConfig.RentalSpots[ClosestShop].coords.z - 0.75 , 'Rent Cars Starting at Just $100 an hour!', 4, 0.08, 0.08, CRConfig.SecondaryColor)
					Citizen.Wait(10)
				else 
					Citizen.Wait(4000)
				end
