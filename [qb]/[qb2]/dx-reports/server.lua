...
local Webhook = 'https://discord.com/api/webhooks//-lFYm4MruPXAH_8YjWZgvGlO0pfZMWgw-4'
local staffs = {}
local FeedbackTable = {}
...
AddEventHandler("dx-reports:NewFeedback", function(data)
	...
	TriggerClientEvent("dx-reports:NewFeedback", -1, newFeedback)
...
AddEventHandler("dx-reports:FetchFeedbackTable", function()
	local staff = hasPermission(source)
	if staff then
		staffs[source] = true
		TriggerClientEvent("dx-reports:FetchFeedbackTable", source, FeedbackTable, staff, oneSync)
	end
end)

-------------------------- ASSIST FEEDBACK

RegisterNetEvent("dx-reports:AssistFeedback")
AddEventHandler("dx-reports:AssistFeedback", function(feedbackId, canAssist)
	if staffs[source] then
		if canAssist then
			...
			if GetPlayerPing(id) > 0 then
				...
				TriggerClientEvent('QBCore:Notify', source, "You are assisting FEEDBACK #"..feedbackId.."!")
				TriggerClientEvent('QBCore:Notify', id, "An admin arrived!")

				if Webhook ~= '' then
					assistFeedbackWebhook(assistFeedback)
				end
			else	
				TriggerClientEvent('QBCore:Notify', id, "That player is no longer in the server!")
			end
			if not FeedbackTable[feedbackId].concluded then
				FeedbackTable[feedbackId].concluded = "assisting"
			end
			TriggerClientEvent("dx-reports:FeedbackConclude", -1, feedbackId, FeedbackTable[feedbackId].concluded)
		end
	end
end)
...
AddEventHandler("dx-reports:FeedbackConclude", function(feedbackId, canConclude)
	if staffs[source] then
...
function hasPermission(id)
	...
	if Config.QBCore then
		local playerGroup = QBCore.Functions.GetPermission(id)

		if playerGroup ~= nil and playerGroup == "god" or playerGroup == "admin" or playerGroup == "mod" then 
			staff = true
		end
	else
