...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob.name == "miner" then
        for k, v in pairs(Config.MiningPositions) do
            table.insert(blips, addBlip(v.coords, 153, 5, Strings['mining']))
        end
        table.insert(blips, addBlip(Config.Sell, 207, 1, Strings['sell_mine']))
    else
...
Citizen.CreateThread(function()
    Citizen.CreateThread(function()
        while true do
            local sleep = 250
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.Sell, true) <= 3.0 and QBCore.Functions.GetPlayerData().job.name == 'miner' then
                sleep = 0
    ...
    while true do
        ...
        if type(closeTo) == 'table' and QBCore.Functions.GetPlayerData().job.name == 'miner'then
