...
CreateThread(function()
  local ret = SqlFunc(Config.Settings['sql'],'fetchAll','SELECT * FROM vl_engine', {})
...
function SaveExhaust(plate, exhaust)
    ...
    local result = SqlFunc(Config.Settings['sql'],'fetchAll','SELECT * FROM vl_engine WHERE TRIM(plate) = @plate', {['@plate'] = plate_})
    if result[1] == nil then
      SqlFunc(Config.Settings['sql'],'execute','INSERT INTO vl_engine (plate, exhaust) VALUES (@plate, @engine)', {
          ['@plate']   = plate,
          ['@engine']   = exhaust,
      })
    elseif result[1] then
      SqlFunc(Config.Settings['sql'],'execute','UPDATE vl_engine SET exhaust = @engine WHERE TRIM(plate) = @plate', {
          ['@plate'] = plate_,
          ['@engine'] = exhaust,
      })
    end
