...
local function RemoveStinger()
    if DoesEntityExist(closestStinger) then
        ...
        if not DoesEntityExist(closestStinger) then
            TriggerServerEvent("dx-spikes:server:pickupspikes")
        end
    end
end
...
-- events
RegisterNetEvent("dx-spikes:client:usespikes", function()
    local ped = PlayerPedId()
    local player = QBCore.Functions.GetPlayerData()
    if not IsPedInAnyVehicle(ped, false) then
        QBCore.Functions.Progressbar("put_spike", 'Placing Spike', 1000, false, false, {
            ...
            TriggerServerEvent('dx-spikes:server:removespikes')
            DeployStinger()
        end, function()
        end)
    else
...
RegisterNetEvent("dx-spikes:client:removespikes", function()
    RemoveStinger()
end)
...
exports['qb-target']:AddTargetModel('p_ld_stinger_s', {
    options = {
        {
            num = 1,
            event = 'dx-spikes:client:removespikes',
