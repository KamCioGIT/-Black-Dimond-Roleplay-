...
QBCore.Functions.CreateUseableItem('spikestrip', function(src)
    print('used')
    TriggerClientEvent("dx-spikes:client:usespikes", src)
end)

RegisterNetEvent('dx-spikes:server:removespikes', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem('spikestrip', 1)
end)

RegisterNetEvent("dx-spikes:server:pickupspikes", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
