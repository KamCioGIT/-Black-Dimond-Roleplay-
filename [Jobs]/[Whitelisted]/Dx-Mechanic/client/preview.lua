...
local function printDifferences(vehicle, properties, newproperties)
	...
	if Config.PreviewPhone then
		if vehlist[1] then
			...
			elseif Config.PhoneMail == "qs" then event = 'lb-phone:server:sendNewMail'
			elseif Config.PhoneMail == "lb" then event = '4p:server:sendNewMail' end
			TriggerServerEvent(event, {
				sender = vehplate,
				subject = veh,
				message = veh..Loc[Config.Lan]["police"].plates..": "..vehplate.."<\n\n><\n\n>"..Loc[Config.Lan]["previews"].changes..#vehlist.."<\n\n> ------------------------------ <\n\n>"..newlist,
				button = {}
			})
		end
	else
...
RegisterNetEvent("jim-mechanic:client:giveList", function(item)
	local list = {}
	local newlist = ""
	for i = 1, #item.info["vehlist"] do newlist = newlist..item.info["vehlist"][i].."<\n\n>" end
	list[#list+1] = { isMenuHeader = true, header = item.info["veh"], txt = Loc[Config.Lan]["police"].plates..": "..item.info["vehplate"].."<\n\n>"..Loc[Config.Lan]["previews"].changes..(#item.info["vehlist"]) }
...
RegisterNetEvent('jim-mechanic:client:Preview:Menu', function()
	...
	if IsPedInAnyVehicle(playerPed, false) then
		local PreviewMenu = {}
			...
			PreviewMenu[#PreviewMenu+1] = { isMenuHeader = true, header = searchCar(vehicle), txt = "Class: "..getClass(vehicle).."<\n\n>"..Loc[Config.Lan]["check"].plate..trim(GetVehicleNumberPlateText(vehicle))..Loc[Config.Lan]["check"].value..searchPrice(vehicle).."<\n\n>"..searchDist(vehicle) }
...
RegisterNetEvent("jim-mechanic:client:Preview:Multi", function(data)
	...
	local modMenu = {
			{ header = searchCar(vehicle), txt = "Class: "..getClass(vehicle).."<\n\n>"..Loc[Config.Lan]["check"].plate..trim(GetVehicleNumberPlateText(vehicle))..Loc[Config.Lan]["check"].value..searchPrice(vehicle).."<\n\n>"..searchDist(vehicle),
...
RegisterNetEvent('jim-mechanic:client:Preview:Livery', function(data)
	...
	if DoesEntityExist(vehicle) then
		...
		if oldlivery == true then
				...
				LiveryMenu[#LiveryMenu + 1] = { header = searchCar(vehicle), txt = "Class: "..getClass(vehicle).."<\n\n>"..Loc[Config.Lan]["check"].plate..trim(GetVehicleNumberPlateText(vehicle))..Loc[Config.Lan]["check"].value..searchPrice(vehicle).."<\n\n>"..searchDist(vehicle),
...
RegisterNetEvent('jim-mechanic:client:Preview:Rims:Check', function()
	...
	local WheelMenu = {}
	WheelMenu[#WheelMenu + 1] = { header = searchCar(vehicle), txt = "Class: "..getClass(vehicle).."<\n\n>"..Loc[Config.Lan]["check"].plate..trim(GetVehicleNumberPlateText(vehicle))..Loc[Config.Lan]["check"].value..searchPrice(vehicle).."<\n\n>"..searchDist(vehicle),
...
RegisterNetEvent('jim-mechanic:client:Preview:Rims:Choose', function(data)
	...
	for i = 1, (GetNumVehicleMods(vehicle, 23) +1) do
		...
		local RimsMenu = {
			...
			{ isMenuHeader = true, header = Loc[Config.Lan]["rims"].menuheader.."<\n\n>("..string.upper(label)..")", txt = "", },
...
RegisterNetEvent('jim-mechanic:client:Preview:Rims:SubMenu', function(data)
	...
	local RimsMenu = {
		...
		{ isMenuHeader = true, header = Loc[Config.Lan]["rims"].menuheader.."<\n\n>("..string.upper(label)..")", txt = Loc[Config.Lan]["common"].amountoption..#data.wheeltable.."<\n\n>"..Loc[Config.Lan]["common"].current..": "..GetLabelText(GetModTextLabel(vehicle, 23, GetVehicleMod(vehicle, 23))), },
