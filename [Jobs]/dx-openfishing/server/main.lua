...
QBCore.Functions.CreateCallback('qb-openfishing:GetItemData', function(source, cb, itemName)
	local retval = false
...
QBCore.Functions.CreateUseableItem("fishrod", function(source, item)
    ...
    TriggerClientEvent('qb-openfishing:tryToFish', source)
end)

RegisterServerEvent('qb-openfishing:receiveFish')
AddEventHandler('qb-openfishing:receiveFish', function(cabin, house)
    local src = source
...
RegisterServerEvent("qb-openfishing:sellFish")
AddEventHandler("qb-openfishing:sellFish", function()
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
		...
		if price > 0 then
			...
			TriggerClientEvent('QBCore:Notify', src, "You have sold your fish", 'success')
		else
			TriggerClientEvent('QBCore:Notify', src, "You don't have anything to sell", 'error')
		end
