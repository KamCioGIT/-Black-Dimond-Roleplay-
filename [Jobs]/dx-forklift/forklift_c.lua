...
local Keys = {
  ...
  local blips = {
    {title="Fork Lift Hanger 1", colour=4, id=290, x = 139.13, y = -3203.16, z = 5.86}
		}
...
local blips = {
    {title="Fork Lift Hanger 2", colour=4, id=290, x = 141.99, y = -3092.58, z = 5.9}
		}
...
Citizen.CreateThread(function()
  ...
  AddEventHandler('tostdostawa:rampanpcdawaj',function(a)
      ...
      if a=='1'then
          if zrespione==true then
              QBCore.Functions.Notify('Complete the previous order!')
              return
          end
          local b=math.random(1,5)
          if b==1 then
              naklejkaAuto=3
              QBCore.Functions.Notify('The driver awaits loading the boards.')
              ZrespPaczuszke('deski')
          elseif b==2 then
              naklejkaAuto=4
              QBCore.Functions.Notify('The driver is waiting for the fish to be loaded.')
              ZrespPaczuszke('lody')
          elseif b==3 then
              naklejkaAuto=6
              QBCore.Functions.Notify('The driver is waiting for the medicine to be loaded.')
              ZrespPaczuszke('leki')
          elseif b==4 then
              naklejkaAuto=2
              QBCore.Functions.Notify('The driver awaits loading Logger Light drinks.')
              ZrespPaczuszke('napoje')
          elseif b==5 then
              naklejkaAuto=1
              QBCore.Functions.Notify('The driver awaits loading the coffee.')
              ZrespPaczuszke('kawa')
          end
          ...
          QBCore.Functions.Notify('The driver is on his way.')
          zrespione=true
          silnikOn=true
          Citizen.Wait(900)
          while silnikOn do
          ...
          QBCore.Functions.Notify('The driver stopped and opened the trunk')
          ...
          QBCore.Functions.Notify('Package loaded, driver wraps up.')
          QBCore.Functions.Notify('You have loaded the package in '..czasikDostaw..' seconds.')
          if czasikDostaw<41 then
              QBCore.Functions.Notify('Bonus '..premia1 ..'$ for fast delivery')
              TriggerServerEvent("tostdostawa:wykonanieMisji",premia1)
              Citizen.Wait(200)
          elseif czasikDostaw>=41 and czasikDostaw<=55 then
              QBCore.Functions.Notify('Bonus '..premia2 ..'$ for fast delivery')
              TriggerServerEvent("tostdostawa:wykonanieMisji",premia2)
              Citizen.Wait(200)
          elseif czasikDostaw>=56 and czasikDostaw<=65 then
              QBCore.Functions.Notify('Bonus '..premia3 ..'$ for fast delivery')
              TriggerServerEvent("tostdostawa:wykonanieMisji",premia3)
              Citizen.Wait(200)
          elseif czasikDostaw>65 then
              QBCore.Functions.Notify('No bonus for quick delivery')
          end
          czasikDostaw=0
          dostawaTimer=false
          ...
          QBCore.Functions.Notify('You can accept the next order')
      elseif a=='2'then
          if zrespione==true then
              QBCore.Functions.Notify('Complete the previous order!')
              return
          ...
          elseif b==3 then
              naklejkaAuto=6
              QBCore.Functions.Notify('The driver is awaiting loading of medications.')
              ZrespPaczuszke('leki')
          elseif b==4 then
              naklejkaAuto=2
              QBCore.Functions.Notify('The driver awaits loading Logger Light drinks.')
      ...
      while true do
          ...
          czasikDostaw = czasikDostaw +1
              if czasikDostaw > 140 then
                ...
                QBCore.Functions.Notify('The order was canceled because you took too long loading')
              end
          else
          Citizen.Wait(2000)
          end	
              ...
              if zlecDist <= 1.0 then
                  ...
                  if IsControlJustPressed(0, Keys['E']) then 
                      if PrzejetyHangar == 1 then
                        ...
                        QBCore.Functions.Notify('The package to be delivered is marked on the GPS, use a forklift.')
                      else
                        QBCore.Functions.Notify('You must take over the hangar')
                      end
                  elseif IsControlJustPressed(0, Keys['G']) then
                      if PrzejetyHangar == 1 then
                        QBCore.Functions.Notify('You already have a hangar')
                      else
  ...
  function PozyczPojazd(a)
      if a=='1'then
          if PojazdWyjety==0 then
              ...
              PojazdWyjety=0
              DeleteEntity(wozek)
              QBCore.Functions.Notify('Your previous vehicle has been deleted, you can retreive a new one')
          end
      elseif a=='2'then
          if PojazdWyjety==0 then
              RequestModel(GetHashKey('forklift'))
  ...
  Citizen.CreateThread(function()
      while true do 
          ...
          if PrzejetyHangar==1 then 
              if b>205.0 then 
                  QBCore.Functions.Notify('Too far away from the warehouse because you lost the owner.')
