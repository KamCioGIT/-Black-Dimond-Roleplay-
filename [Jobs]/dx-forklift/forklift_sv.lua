...
AddEventHandler('tostdostawa:przejmijHangar', function(ktory)
	if ktory == '1' then
		if hangar1ID == nil then
			hangar1ID = source
			TriggerClientEvent('QBCore:Notify', source, 'You are taking over the hangar.')
			TriggerClientEvent('QBCore:Notify', source, 'Now only you can perform orders here.')
			TriggerClientEvent("tostdostawa:maszHangar", source)
		else
			TriggerClientEvent('QBCore:Notify', source, 'This hangar is already taken by ID '..hangar1ID)
		end
	elseif ktory == '2' then
		if hangar2ID == nil then
			...
			TriggerClientEvent('QBCore:Notify', source, 'This hangar is already taken by ID '..hangar2ID)
		end
	end
end)
...
AddEventHandler('tostdostawa:wykonanieMisji', function(premia)
	...
	if premia == 'nie' then
		...
		TriggerClientEvent('QBCore:Notify', _source, 'You get $'..LosujSiano..' for delivery')
		Wait(2500)
	else
