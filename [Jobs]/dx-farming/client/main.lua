			...
			if not f then
				...
				if not cowmilking or not track and not oranges then 
					for i = 1, #Config.OrangeFarm do		
						...
						if orangedis < 5 then 
							Draw3DText(Config.OrangeFarm[i].x, Config.OrangeFarm[i].y, Config.OrangeFarm[i].z, '[E] - Start Picking Apples', 4, 0.08, 0.08, Config.SecondaryColor)
							if IsControlJustReleased(0, 38) and not oranges then
								oranges = true
								PickOrange()
							elseif IsControlJustReleased(0, 38) and oranges then
								QBCore.Functions.Notify('Seems Like you just got Some Apples! Wait for a Few Seconds before Trying again!')
							end
						end
					end		
					sleep = 5	
...
Citizen.CreateThread(function()
	while true do
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				...
				QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
					if result then
						...
						QBCore.Functions.Notify('You need a box to continue the process')
					end
				end, 'box')
			end
		...
		elseif GetDistanceBetweenCoords(coords, Config.CircleZones.OrangePack.coords, true) < 3 then
			...
			if not isProcessing then
				Draw3DText(nowcoords.x, nowcoords.y,nowcoords.z, '[E] - Pack Apples', 4, 0.08, 0.08, Config.SecondaryColor)
			end
			if IsControlJustReleased(0, 38) and not isProcessing then
				-- QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
		...
		if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.SellLocation.x, Config.SellLocation.y, Config.SellLocation.z, true) < 5.0 then
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.SellLocation.x, Config.SellLocation.y, Config.SellLocation.z, true) < 1.5 then
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						...
						DrawText3D(Config.SellLocation.x, Config.SellLocation.y, Config.SellLocation.z, "Don's Country Store - you don t have anything to sell..")
					end
			end
		end
		if not inRange then
...
function PickOrange()
	...
	QBCore.Functions.Progressbar("search_register", "Picking Up Apples!", 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
