...
Citizen.CreateThread(function()
	...
	local rzeznia = AddBlipForCoord(portionX, portionY, portionZ)
		...
		SetBlipColour (rzeznia, 59)
		SetBlipAsShortRange(rzeznia, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('Chicken Slauter House')
...
--
		if dist <= 0.5 then
			...
			TriggerServerEvent("og-chickenjob:startChicken")
			LapChicken()
			end			
		end
	end
...
if share == true then
	...
	TriggerServerEvent("og-chickenjob:getNewChicken")
	end
...
function packingg(stanowisko)
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
		if result then
			...
			QBCore.Functions.Progressbar("wash-", "Packing..", 10000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
			TriggerServerEvent("og-chickenjob:getpackedChicken",2)
...
function portioning(position)
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
		if result then
			...
			QBCore.Functions.Progressbar("Cut-", "Slaughtering..", 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			TriggerServerEvent("og-chickenjob:getcutChicken", 1)
			FreezeEntityPosition(GetPlayerPed(-1),false)
...
function packedsell()
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
		if result then
			...
			QBCore.Functions.Progressbar("Cut-", "Selling..", 30000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
				TriggerServerEvent("og-chickenjob:sell",3)
				ClearPedTasks(GetPlayerPed(-1))
				DeleteEntity(prop)
			end, function() -- Cancel
...
-- {{2435.53, 4975.82, 46.57 , 0}, {2437.3, 4974.1, 46.83, 0},{0,0,0}, "Press [E] To Go in"},
-- {{329.94, -601.08, 43.28 , 0}, {341.02, -580.97, 28.8, 248.72},{0,0,0}, "Press [E] To Go"},
-- {{339.58, -584.61, 28.8 , 254.6}, {340.67, -580.49, 74.16, 41.01},{0,0,0}, "Press [E] To Go"}
