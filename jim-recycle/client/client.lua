...
RegisterNetEvent('jim-recycle:Selling:Menu', function()
	...
	local sellMenu = {
		...
		{ icon = "fas fa-circle-xmark", header = Loc[Config.Lan].menu["close"], txt = "", params = { event = "jim-recycle:CloseMenu" } } }
	for k, v in pairsByKeys(Config.Prices) do
		sellMenu[#sellMenu+1] = {
			disabled = not HasItem(k, 1),
			icon = k,
			header = QBCore.Shared.Items[k].label, --"<img src=nui://"..Config.img..QBCore.Shared.Items[k].image.." width=30px onerror='this.onerror=null; this.remove();'> "..
