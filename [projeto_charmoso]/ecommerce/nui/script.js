...
function renderCars() {
    ...
    main_filter.innerHTML = `
        ...
        <div id="withdrawimgdiv" class = 'moneyStatus-container' onClick="withdrawMoney()">
            <main>
                <img id="withdrawimg" src="images/withdraw.png" alt="">
...
function getEcommercePageData(searchValue,type,page,maxPrice,minPrice,productclass) {
    document.querySelector('.all').style.filter = 'blur(10px)'
    document.querySelector('.loadingContainer').style.display = 'flex'
    $.post("http://ecommerce/getEcommercePageData", JSON.stringify({search:searchValue,type:type,page:page,maxPrice:maxPrice,minPrice:minPrice,productclass:productclass}), function (datab) {
        if(datab){
            var c = 0
            if(type == 0) {
                pageNumberCars = page
