...
RegisterNUICallback('post', function(body, cb)
	if cooldown == nil then
		...
		elseif body.event == "spawnTruck" and loading then
			...
			closeUI()
		elseif body.event == "repairTruck" and (truck and not rent_truck) then
			exports['lc_utils']:notify("error",Utils.translate('store_truck'))
		elseif body.event == "sellTruck" and (truck and not rent_truck) then
...
AddEventHandler('truck_logistics:spawnTruck', function(truck_data)
	...
	truck_data.properties = json.decode(truck_data.properties)
	truck_data.properties.plate = truck_data.properties.plate or Utils.translate('truck_plate')..tostring(math.random(1000000, 9999999))
					...
					if distance <= 20.0 then
						...
						if distance <= 5.0 then
							...
							if IsControlJustPressed(0,38) and IsEntityAVehicle(truck) then
								...
								Utils.Framework.removeVehicleKeys(truck)
...
AddEventHandler('truck_logistics:startContract', function(key,contract_data,location)
	...
	if not IsEntityAVehicle(trailer) then
		Utils.Framework.removeVehicleKeys(trailer)
	...
	if contract_data.contract_type == 0 then
		...
		local properties = { fuelLevel = 100.0, plate = Utils.translate('truck_plate')..tostring(math.random(1000000, 9999999)) }
