        ...
        if KidNapping['sceneFinish'] then
            ...
            if cameraDist <= 3.0 then
                ...
                if IsControlJustPressed(0, 38) then
                    ...
                    QBCore.Functions.Notify(Strings['go_laptop']) 
                end
            end
        end
...
function CloseEyes()
    QBCore.Functions.TriggerCallback('kidnapping:server:checkQueryRoom', function(status)
        if not status then
            ...
            QBCore.Functions.Notify(Strings['query_room_busy'], 'error') 
        end
    end)
end
...
function KidnappingStart()
    QBCore.Functions.TriggerCallback('kidnapping:server:checkTime', function(time)
        if time then
            query = true
            QBCore.Functions.Notify(Strings['info_1']) 
            QBCore.Functions.Notify(Strings['info_2']) 
            QBCore.Functions.Notify(Strings['info_3']) 
            local ped = PlayerPedId()
            ...
            Citizen.CreateThread(function()
                while query do
                    ...
                    if dist2 >= 150.0 then
                        QBCore.Functions.Notify(Strings['mission_failed2'], 'error') 
                        break
                    end
                    ...
                    if IsEntityPlayingAnim(KidNapping['kidnappedPed'], 'missminuteman_1ig_2', 'handsup_enter', 1) then
                        ...
                        if GetEntityType(y) == 2 then
                            ShowHelpNotification(Strings['go_query'])
...
function FirstAnimation()
    ...
    exports['ps-dispatch']:Kidnap()
    --AlertPolice(GetEntityCoords(PlayerPedId()))
    SecondAnimation()
end
...
function AlertPolice(coords)
            local data = {displayCode = '10-33', description = 'Kidnapping Attempt', isImportant = 0, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Kidnap Attempt Reported'}
    ...
    --[[if QBCore.Functions.GetPlayerData().job.name == "police" then
        ...
        TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
            ...
            details = {
                [1] = {
                    ...
                    detail = "Someone kidnapping a citizen",
                }
            },
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("10-90: Kidnapping Attempt")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
