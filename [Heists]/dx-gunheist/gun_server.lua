...
RegisterServerEvent('dx-gunheist:wonHacking')
AddEventHandler('dx-gunheist:wonHacking', function()
	local src = source
...
RegisterServerEvent('dx-gunheist:server:useDecryptionKey')
AddEventHandler('dx-gunheist:server:useDecryptionKey', function(item)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Player then
		...
		if keySlot.amount > 0 then
			...
			if keys[activeKeys[keysUsed + 1]].itemName == item then
				keysUsed = keysUsed + 1
				if keysUsed == 3 then
					TriggerClientEvent('dx-gunheist:hackingMinigame', src)
				else
...
RegisterServerEvent('dx-gunheist:createHeistLoop')
AddEventHandler('dx-gunheist:createHeistLoop', function()
	heistId = heistId + 1
	...
	if heistId == thisId then
		...
		if heistId == thisId then
			TriggerEvent('dx-gunheist:createHeistLoop')
		end
	end
end)

RegisterServerEvent('dx-gunheist:obtainDrops')
AddEventHandler('dx-gunheist:obtainDrops', function()
	TriggerClientEvent('dx-gunheist:obtainDrops', source, activeDrops)
end)
...
RegisterServerEvent('dx-gunheist:pickUpDrop')
AddEventHandler('dx-gunheist:pickUpDrop', function(dropId)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if activeDrops[dropId] then
		...
		if Player then
			if drillSlot ~= nil and drillSlot.amount >= 1 then
				...
				TriggerClientEvent('dx-gunheist:manipulateDrop', -1, dropId)
			else
...
RegisterServerEvent('dx-gunheist:hackingCompleted')
AddEventHandler('dx-gunheist:hackingCompleted', function()
	local src = source
	...
	if not weaponDropped then
		...
		if item then
			...
			TriggerClientEvent('dx-gunheist:manipulateDrop', -1, dropIds, activeDrops[dropIds])
			TriggerClientEvent('dx-gunheist:createBlipOnDrop', src, dropIds, item.label)
			TriggerEvent('dx-gunheist:createHeistLoop')
		end
	end
end)
...
QBCore.Functions.CreateUseableItem('electronickit', function(source, item) 
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		TriggerClientEvent('dx-gunheist:useChip', source)
	end
end)
...
QBCore.Functions.CreateUseableItem("greenchip", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		...
		TriggerClientEvent('dx-gunheist:useDecryptionKey', source,  'greenchip')
    end
end)
...
QBCore.Functions.CreateUseableItem("redchip", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		local itemName = item.name
		TriggerClientEvent('dx-gunheist:useDecryptionKey', source,  'redchip')
    end
end)
...
QBCore.Functions.CreateUseableItem("bluechip", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		local itemName = item.name
		TriggerClientEvent('dx-gunheist:useDecryptionKey', source, 'bluechip')
    end
end)
