...
QBCore.Functions.CreateCallback("osm-atmrobbery:getHackerDevice",function(source,cb)
	...
	if xPlayer.Functions.GetItemByName("gatecrack") and xPlayer.Functions.GetItemByName("drill") then
		cb(true)
	else
		cb(false)
		TriggerClientEvent('QBCore:Notify', source, "You need an Gate Crack and a Drill to proceed.")
	end
end)
...
QBCore.Functions.CreateUseableItem('gatecrack', function(source)
