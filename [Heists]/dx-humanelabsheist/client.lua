...
AddEventHandler('humanelabsheist:client:policeAlert', function(targetCoords)
            local data = {displayCode = '10-33', description = 'Chemical Robbery Attempt', isImportant = 0, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Humanelabs Heist is in progress'}
            local dispatchData = {dispatchData = data, caller = 'Alarm', coords = vector3(targetCoords.x, targetCoords.y, targetCoords.z)}
        ...
        if status then
            QBCore.Functions.TriggerCallback('humanelabsheist:server:checkTime', function(time)
                if time then
                    ...
                    TriggerServerEvent("humanelabsheist:server:adddiving")
                    Citizen.CreateThread(function()
                        while true do
                            local ped = PlayerPedId()
                            local pedCo = GetEntityCoords(ped)
