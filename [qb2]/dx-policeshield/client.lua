...
RegisterNetEvent('dx-policeshield:client:toggleShield')
AddEventHandler('dx-policeshield:client:toggleShield', function(type)
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    if vehicle ~= 0 and vehicle ~= nil then
        QBCore.Functions.Notify(translations.outsideVehicle, 'error')
        return
    end
    if shield.active then
        ...
        if type == nil then QBCore.Functions.Notify(translations.noArguments, 'error') return end
        createShield(type)
    end
end)
...
function removeShield(notify)
    if DoesEntityExist(shield.prop) then
        ...
        if notify then
            QBCore.Functions.Notify(translations.shieldRemoved, 'error')
        end
    end
end
