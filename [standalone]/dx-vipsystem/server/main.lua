...
QBCore.Functions.CreateCallback('dx-donateSystem:getPlayerDetails', function(source, cb)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local charInfo = xPlayer.PlayerData.charinfo
    local callbackData = {}
    if dx.Mysql == "oxmysql" then 
        local result = MySQL.query.await('SELECT * FROM dx_donatesystem WHERE citizenid = ?', { citizenId })
        ...
        cb(callbackData)
    elseif dx.Mysql == "mysql-async" then
        MySQL.Async.fetchAll("SELECT * FROM dx_donatesystem WHERE citizenid = @citizenid", {
            ["@citizenid"] = citizenId
        }, function (result)
            if result[1] == nil then 
                callbackData = {
...
QBCore.Functions.CreateCallback('dx-donateSystem:sendInput', function(source, cb, data)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local inputData = data.input
    if dx.Mysql == "oxmysql" then
        local result = MySQL.query.await('SELECT * FROM dx_donatesystem_codes WHERE code = ?', { inputData })
        if result[1] ~= nil then 
            MySQL.query('DELETE FROM dx_donatesystem_codes WHERE code = ?', {inputData})
            local result2 = MySQL.query.await('SELECT * FROM dx_donatesystem WHERE citizenid = ?', { citizenId })
            if result2[1] ~= nil then 
                MySQL.update('UPDATE dx_donatesystem SET credit = credit + '..result[1].credit..' WHERE citizenid = ?', { citizenId })
            else
                MySQL.insert('INSERT INTO dx_donatesystem (citizenid, credit) VALUES (?, ?)', {
                    citizenId,
                    result[1].credit,
                })
            end
    ...
    elseif dx.Mysql == "mysql-async" then 
        MySQL.Async.fetchAll("SELECT * FROM dx_donatesystem_codes WHERE code = @code", {
            ["@code"] = inputData
        }, function (result)
            if result[1] ~= nil then 
                MySQL.Async.execute('DELETE FROM dx_donatesystem_codes WHERE code = @code', {['@code'] = inputData})
                MySQL.Async.fetchAll("SELECT * FROM dx_donatesystem WHERE citizenid = @citizenid", {
                    ["@citizenid"] = citizenId
                }, function (result2)
                    if result2[1] ~= nil then 
                        MySQL.Async.execute("UPDATE dx_donatesystem SET credit = credit + @credit WHERE citizenid = @citizenid", {
                            ['@citizenid'] = citizenId,
                            ['@credit'] = result[1].credit
                        })
                    else
                        MySQL.Async.execute('INSERT INTO dx_donatesystem (citizenid, credit) VALUES (@citizenid, @credit)', {
                            ["@citizenid"] = citizenId,
                            ["@credit"] = result[1].credit,
                        })
                    end
...
QBCore.Functions.CreateCallback('dx-donateSystem:getVehicle', function(source, cb, veri)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local data = nil
    ...
    if data2.itemInfo.vehType == "car" then
        for k, v in pairs(dx.Vehicles) do
    ...
    elseif data2.itemInfo.vehType == "air" then
        for k, v in pairs(dx.Helicopters) do
            if v.id == veri.itemInfo.id then 
                data = v
    ...
    if data then
        ...
        if data2.extra ~= "undefined" then 
            ...
            costCredit = costCredit + dx.CustomPlatePriceOnBuyVehicle
        end
        if dx.Mysql == "oxmysql" then
            local result = MySQL.query.await('SELECT * FROM dx_donatesystem WHERE citizenid = ?', { citizenId })
            if result[1] ~= nil then 
                if result[1].credit >= costCredit then
                    if plateCheck then 
                        ...
                        if resultPlate[1] == nil then 
                            MySQL.update('UPDATE dx_donatesystem SET credit = credit - ? WHERE citizenid = ?', { costCredit, citizenId })
                    ...
                    if result[1].credit >= costCredit then
                        if plateCheck then 
                            MySQL.Async.fetchAll('SELECT * FROM player_vehicles WHERE plate = @plate', {
                                ...
                                if resultPlate[1] == nil then 
                                    MySQL.Async.execute("UPDATE dx_donatesystem SET credit = credit - "..costCredit.." WHERE citizenid = @citizenid", {["@citizenid"] = citizenId})
...
QBCore.Functions.CreateCallback('dx-donateSystem:getWeapon', function(source, cb, veri)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local data = nil
    for k, v in pairs(dx.Weapons) do
        if v.id == veri.itemInfo.id then 
            data = v
            break
        end
    ...
    if data then
        ...
        if dx.Mysql == "oxmysql" then
            ...
            if result[1] ~= nil then 
                if result[1].credit >= costCredit then
                    MySQL.update('UPDATE dx_donatesystem SET credit = credit - ? WHERE citizenid = ?', { costCredit, citizenId })
                    if dx.WeaponType == "item" then 
                        xPlayer.Functions.AddItem(rewardItem, 1)
                    elseif dx.WeaponType == "weapon" then 
...
QBCore.Functions.CreateCallback('dx-donateSystem:getItem', function(source, cb, veri)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local data = nil
    for k, v in pairs(dx.Items) do
        if v.id == veri.itemInfo.id then 
            data = v
            break
        end
...
QBCore.Functions.CreateCallback('dx-donateSystem:getMoney', function(source, cb, veri)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local data = nil
    for k, v in pairs(dx.Money) do
        if v.id == veri.itemInfo.id then 
            data = v
            break
        end
...
QBCore.Functions.CreateCallback('dx-donateSystem:getCustomPlate', function(source, cb, veri, plate)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local data = nil
    for k, v in pairs(dx.Customize) do
        if v.id == veri.itemInfo.id then 
            data = v
            break
        end
...
QBCore.Functions.CreateCallback('dx-donateSystem:getPrivNumber', function(source, cb, veri)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local data = nil
                        ...
                        if dx.Phone == "qb-phone" then 
                            ...
                            if checkPhone == 0 then
                                ...
                                if charInfo.phone then 
                                    ...
                                    cb(true)
                                    DropPlayer(_source, "[dx-donateSystem] Phone number changed!")
...
RegisterCommand('purchase_donate_credit', function(source, args)
    ...
    if src == 0 then
        ...
        inProgress = true
        if dx.Mysql == "oxmysql" then
            local result = MySQL.query.await('SELECT * FROM dx_donatesystem_codes WHERE code = ?', { tbxid })
            if result[1] == nil then
                MySQL.insert('INSERT INTO dx_donatesystem_codes (code, credit) VALUES (?, ?)', { tbxid, credit })
                SendToDiscord("Code: ``"..tbxid.."``\nCredit: ``"..credit.."``\nsuccessfuly into your database!")
            end
            inProgress = false  
        elseif dx.Mysql == "mysql-async" then
            MySQL.Async.fetchAll('SELECT * FROM dx_donatesystem_codes WHERE code = @code', {['@code'] = tbxid}, function(result)
                if result[1] == nil then
                    MySQL.Async.execute('INSERT INTO dx_donatesystem_codes (code, credit) VALUES (@code, @credit)', {
                        ["@code"] = tbxid,
                        ["@credit"] = credit,
                    })
                    SendToDiscord("Code: ``"..tbxid.."``\nCredit: ``"..credit.."``\nsuccessfuly into your database!")
    ...
    if resource == GetCurrentResourceName() then
        if dx.UseTebex then 
            ...
            if tebexConvar == '' or tebexConvar == nil then
                ...
                print('^1////////////////////////////////////////////////////////////////////////////////////////////////////////////')
                print('dx-donateSystem: Tebex Secret Missing please set in server.cfg and try again. Script will not work correctly.')
                shouldStop = true
            end
        end
	end
...
local DISCORD_NAME = "dx-donateSystem"
